...
if(item){
    ...
    $('#reviews-detalhe').html(item.reviews + ' Visualizações')
    $('#descricao-detalhe').html(item.descricao)
    $('#preco-detalhe').html(item.preco.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'}))
    $('#precopromocional-detalhe').html(item.preco_promocional.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'}))

    var tabelaDetalhes = $('#tabdetalhes')

    item.detalhes.forEach(detalhe =>{
        var linha = `<tr>
                        <th>${detalhe.caracteristica}</th>
                        <th>${detalhe.detalhes}</th>
                    </tr>`
